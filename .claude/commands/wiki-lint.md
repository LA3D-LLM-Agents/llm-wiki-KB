...
You are running a health check on the wiki at `wiki/llm-wiki-KB.wiki/`. Defer to `SCHEMA_llm-wiki-KB.md` for the precise conventions.
...
6. For accepted fixes, apply them with cross-reference repair in both directions, update `index_llm-wiki-KB.md` as needed, and append a `## [YYYY-MM-DD] lint | Subject` entry to `log_llm-wiki-KB.md`. The first bullet of that entry is the attribution line `- by: <name> via claude-code`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). See "Log Entry Attribution" in `SCHEMA_llm-wiki-KB.md`.
...
8. **Finish the cycle.** Commit in the wiki's own git repo in two steps, without asking. One commit per log entry keeps `git blame` on the log a faithful per-entry record (see "Log Entry Attribution" in SCHEMA):
    ```
    git -C wiki/llm-wiki-KB.wiki add <lint-fix-and-index-files-by-name>
    git -C wiki/llm-wiki-KB.wiki commit -m "lint: <summary>"
    git -C wiki/llm-wiki-KB.wiki add log_llm-wiki-KB.md
    git -C wiki/llm-wiki-KB.wiki commit -m "log: lint <summary>"
    ```
