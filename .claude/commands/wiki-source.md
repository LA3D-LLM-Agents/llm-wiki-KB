...
Full procedure: see `.claude/skills/wiki-source.md` and `wiki/llm-wiki-KB.wiki/SCHEMA_llm-wiki-KB.md`. Summary:
...
7. Update `index_llm-wiki-KB.md` under "Source summaries".
8. Append a `## [YYYY-MM-DD] ingest | Source title` entry to `log_llm-wiki-KB.md`. The first bullet is the attribution line `- by: <name> via claude-code`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the ingest. See "Log Entry Attribution" in `SCHEMA_llm-wiki-KB.md`.
