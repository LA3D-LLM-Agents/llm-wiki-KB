...
Full procedure: see `.claude/skills/wiki-experiment.md` and `wiki/llm-wiki-KB.wiki/SCHEMA_llm-wiki-KB.md`. Summary:
...
7. Update `index_llm-wiki-KB.md` with one-line descriptions of new pages.
...
11. **Finish the cycle.** Commit in the wiki's own git repo in two steps, without asking. One commit per log entry keeps `git blame` on the log a faithful per-entry record (see "Log Entry Attribution" in SCHEMA):
    ```
    git -C wiki/llm-wiki-KB.wiki add <page-and-index-files-by-name>
    git -C wiki/llm-wiki-KB.wiki commit -m "<descriptive message>"
    git -C wiki/llm-wiki-KB.wiki add log_llm-wiki-KB.md
    git -C wiki/llm-wiki-KB.wiki commit -m "log: <descriptive message>"
    ```
